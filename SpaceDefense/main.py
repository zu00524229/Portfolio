...
from rank import input_name_screen, draw_rank_screen, submit_score
...
def draw_init():
    ...
    draw_text(screen, '太空保衛戰!', 64, WIDTH/2, HEIGHT/4, font_name) # 畫出標題
    draw_text(screen, 'A D 移動飛船 空白鍵發射子彈', 22, WIDTH/2, HEIGHT/2, font_name) # 畫出提示文字
    draw_text(screen, '任意鍵開始遊戲', 18, WIDTH/2, HEIGHT*3/4, font_name)
...
while running:
    ...
    if player.lives == 0 and not(death_expl.alive()):
        name = input_name_screen(screen, background_img, score, WIDTH, HEIGHT, font_name)
        # print("要送出的 name 和 score：", name, score)
        submit_score(name, score)
        draw_rank_screen(screen, background_img, score, WIDTH, HEIGHT, font_name)
        show_init = True
    ...
    draw_text(screen, str(score), 18, WIDTH/2, 10, font_name) # 畫出分數 
