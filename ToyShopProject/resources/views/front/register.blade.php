  ...
  <div class="container mt-5 mb-5" style="max-width: 600px; text-align: center;">
    <div class="container" style="max-width: 500px; width: 100%; border-radius: 15px;">
      <div class="row" style="background-color: rgb(250, 249, 249); padding: 30px; border-radius: 12px; box-shadow: 0px 4px 12px rgba(0, 0, 0, 0.25);">
        ...
        <form method="POST" action="/front/postRegister" id="registrationForm">
          @csrf <!-- Laravel CSRF 保護 -->

          <!-- 玩家姓名輸入框 -->
          <div class="form-floating mb-3">
            <input type="text" class="form-control" id="name" name="name" placeholder="請輸入玩家姓名" value="{{ old('name') }}" required>
            <label for="name">玩家姓名</label>
          </div>

          <!-- 玩家暱稱輸入框 -->
          <div class="form-floating mb-3">
            <input type="text" class="form-control" id="nickName" name="nickName" placeholder="請輸入玩家暱稱" value="{{ old('nickName') }}" required>
            <label for="nickName">玩家暱稱</label>
          </div>

          <!-- 玩家帳號輸入框 -->
          <div class="form-floating mb-3">
            <input type="text" class="form-control" id="account" name="account" placeholder="請輸入玩家帳號" value="{{ old('account') }}" required>
          ...
          <div class="form-floating mb-3 position-relative">
            <input type="password" class="form-control" id="password" name="password" placeholder="請輸入玩家密碼" required>
          ...
          <!-- 確認密碼輸入框 -->
          <div class="form-floating mb-3 position-relative">
            <input type="password" class="form-control" id="passwordConfirmation" name="passwordConfirmation" placeholder="再次輸入密碼" required>
          ...
          <!-- 手機號碼輸入框 -->
          <div class="form-floating mb-3">
            <input type="text" class="form-control" id="telephone" name="telephone" placeholder="請輸入手機號碼" value="{{ old('telephone') }}" required pattern="09\d{8}">
          ...
          <!-- 住址輸入框 -->
          <div class="form-floating mb-3">
            <input type="text" class="form-control" id="address" name="address" placeholder="請輸入住址" value="{{ old('address') }}" required>
          ...
          <!-- 電子郵件輸入框 -->
          <div class="form-floating mb-3">
            <input type="email" class="form-control" id="email" name="email" placeholder="請輸入信箱" value="{{ old('email') }}" required>
          ...
          <!-- 生日輸入框 -->
          <div class="form-floating mb-3">
            <input type="date" class="form-control" id="birthdate" name="birthdate" placeholder="請輸入生日" value="{{ old('birthdate') }}" required>
          ...
          <!-- 註冊按鈕 -->
          <div class="d-grid">
            <button type="submit" class="btn btn-primary btn-lg" id="registerButton" disabled>註冊</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div class="text-center mt-4">
      <p class="mb-0">已經有帳號了嗎？ <a href="/front/login" class="text-primary fw-bold">立即登入</a></p>
    </div>
  </div>
</section>
